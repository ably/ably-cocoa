    ...
    func test__111__Connection__with_fixture_messages__should_encode_and_decode_fixture_messages_as_expected() throws {
        ...
        for fixtureMessage in messages {
            ...
            waitUntil(timeout: testTimeout) { done in
                ...
                client.internal.rest.execute(request as URLRequest, withAuthOption: .on, wrapperSDKAgents: nil, completion: { _, _, err in
                    if let err = err {
                        fail("\(err)")
                    }
                })
            ...
            waitUntil(timeout: testTimeout) { done in
                channel.publish([message]) { err in
                    ...
                    client.internal.rest.execute(request as URLRequest as URLRequest, withAuthOption: .on, wrapperSDKAgents: nil, completion: { _, data, err in
                        if let err = err {
                            fail("\(err)")
                            done()
                            return
    ...
    func test__112__Connection__with_fixture_messages__should_send_messages_through_raw_JSON_POST_and_retrieve_equal_messages_through_MsgPack_and_JSON() throws {
        ...
        for fixtureMessage in messages {
            waitUntil(timeout: testTimeout) { done in
                ...
                restPublishClient.internal.execute(request as URLRequest, withAuthOption: .on, wrapperSDKAgents: nil, completion: { _, _, err in
                    if let err = err {
                        fail("\(err)")
                    }
                })
        ...
        for fixtureMessage in messages {
            ...
            for restPublishChannel in [restPublishChannelMsgPack, restPublishChannelJSON] {
                ...
                waitUntil(timeout: testTimeout) { done in
                    ...
                    request.httpMethod = "GET"
                    request.allHTTPHeaderFields = ["Accept": "application/json"]
                    restRetrieveClient.internal.execute(request as URLRequest, withAuthOption: .on, wrapperSDKAgents: nil, completion: { _, data, err in
                        if let err = err {
                            fail("\(err)")
                            done()
                            return

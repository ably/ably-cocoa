...
class PushTests: XCTestCase {
    ...
    func test__018__LocalDevice__when_alternative_device_token_available_update_push_details_on_the_Ably_server_by_triggering_GotPushDeviceDetails() throws {
        ...
        func requestAlternativeDeviceToken(_ completion: @escaping (Data?, Error?) -> Void) {
            delay(0.1) {
                completion(TestLocationDeviceToken.tokenData, nil)
            }
        }
        
        waitUntil(timeout: testTimeout) { done in
            pushRegistererDelegate.onDidActivateAblyPush = { _ in
                requestAlternativeDeviceToken() { data, _ in
                    ARTPush.didRegisterForLocationNotifications(withDeviceToken: data!, rest: rest)
                    done()
                }
            }
            rest.push.activate()

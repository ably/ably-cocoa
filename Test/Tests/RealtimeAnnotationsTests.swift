...
class RealtimeAnnotationsTests: XCTestCase {
    ...
    func test__publish_delete_and_get_annotations() throws {
        ...
        waitUntil(timeout: testTimeout) { done in
            realtimeChannel.annotations.delete(for: receivedMessage, annotation: createdAnnotation) { error in
                XCTAssertNil(error)
                done()
            }
        }
    ...
    func test__publish_annotation_exceeding_max_message_size() throws {
        ...
        // Create an annotation with data larger than maxMessageSize
        let largeString = String(repeating: "f", count: realtimeClient.connection.maxMessageSize + 100)
        
        waitUntil(timeout: testTimeout) { done in
            // Create annotation with large data
            let annotation = ARTAnnotation(
                id: nil,
                action: .create,
                clientId: nil,

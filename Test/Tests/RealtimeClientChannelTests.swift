...
private var rtl6c2TestsClient: Realtime!
...
private var rtl6c4TestsClient: Realtime!
...
class RealtimeClientChannelTests: XCTestCase {
    ...
    func test__006__Channel__EventEmitter__channel_states_and_events__can_emit_an_UPDATE_event() throws {
        ...
        waitUntil(timeout: testTimeout) { done in
            ...
            let attachedMessage = ProtocolMessage()
    ...
    func test__011b__Channel__EventEmitter__channel_states_and_events__ChannelStateChange_will_contain_a_resumed_boolean_attribute_with_value__true__if_the_bit_flag_RESUMED_was_included_for_recovered_connection() throws {
        ...
        let recoveredClient = Realtime(options: options)
    ...
    func test__202__Channel__attach__protocol_message_channelSerial_must_be_set_to_channelSerial_of_the_most_recent_protocol_message_or_omitted_if_no_previous_protocol_message_received() throws {
        ...
        let latestAttachProtocolMessage: () throws -> ProtocolMessage = {
    ...
    func test__048__Channel__attach__attach_resume__should_encode_correctly_the_AttachResume_flag() throws {
        ...
        let client1 = Realtime(options: options)
        defer { client1.dispose(); client1.close() }
        ...
        let client2 = Realtime(options: options)
        defer { client2.dispose(); client2.close() }
    ...
    func test__079__Channel__publish__Connection_state_conditions__the_message__should_NOT_be_queued_instead_it_should_be_published_if_the_channel_is__INITIALIZED() throws {
        ...
        rtl6c2TestsClient.connect()
        XCTAssertEqual(rtl6c2TestsChannel.state, RealtimeChannelState.initialized)
    ...
    func test__080__Channel__publish__Connection_state_conditions__the_message__should_NOT_be_queued_instead_it_should_be_published_if_the_channel_is__ATTACHING() throws {
        ...
        waitUntil(timeout: testTimeout) { done in
            ...
            XCTAssertEqual(rtl6c2TestsChannel.state, RealtimeChannelState.attaching)
            rtl16c2TestsPublish(done)
            XCTAssertEqual(rtl6c2TestsClient.internal.queuedMessages.count, 0)
            XCTAssertEqual((rtl6c2TestsClient.internal.transport as! TestProxyTransport).protocolMessagesSent.filter { $0.action == .message }.count, 1)
        }
    ...
    func test__081__Channel__publish__Connection_state_conditions__the_message__should_NOT_be_queued_instead_it_should_be_published_if_the_channel_is__ATTACHED() throws {
        ...
        XCTAssertEqual(rtl6c2TestsChannel.state, RealtimeChannelState.attached)
    ...
    func test__088__Channel__publish__message_bundling__Messages_are_delivered_using_a_single_ProtocolMessage_where_possible_by_bundling_in_all_messages_for_that_channel() throws {
        ...
        for i in 1 ... maxMessages {
            messages.append(Message(name: "total number of messages", data: "message\(i)"))
        }
        waitUntil(timeout: testTimeout) { done in
    ...
    func test__094__Channel__publish__message_bundling__Messages_with_differing_clientId_values_must_not_be_bundled_together__messages_bundled_by_the_user_are_posted_in_a_single_protocol_message_even_if_they_have_mixed_clientIds() throws {
        ...
        var messages = [Message]()
        for i in 1 ... 3 {
            messages.append(Message(name: "name\(i)", data: "data\(i)", clientId: "clientId\(i)"))
        }
    ...
    func skipped__test__090__Channel__publish__message_bundling__should_only_bundle_messages_when_it_respects_all_of_the_constraints() throws {
        ...
        Default.setMaxMessageSize(256)
        defer { Default.setMaxMessageSize(defaultMaxMessageSize) }
        ...
        channelOne.publish([
            Message(name: "b", data: ["expectedBundle": 1]),
            Message(name: "c", data: ["expectedBundle": 1]),
        ])
        ...
        channelOne.publish([Message(name: "f", data: ["expectedBundle": 3])])
        // RTL6d2
        ...
        // RTL6d7
        channelOne.publish([Message(id: "bundle_m", name: "m", data: ["expectedBundle": 9])])
    ...
    func test__091__Channel__publish__message_bundling__should_publish_only_once_on_multiple_explicit_publish_requests_for_a_given_message_with_client_supplied_ids() throws {
        ...
        channel.publish([Message(id: "some_msg_id", name: "event0", data: "")])
        channel.publish([Message(id: "some_msg_id", name: "event0", data: "")])
        channel.publish([Message(id: "some_msg_id", name: "event0", data: "")])
    ...
    func test__098__Channel__publish__expect_either__allows_name_to_be_null() throws {
        ...
        let rawMessagesSent = rawProtoMsgsSent.filter { $0["action"] as! UInt == ProtocolMessageAction.message.rawValue }
    ...
    func test__117__Channel__history__should_support_all_the_same_params_as_Rest() throws {
        ...
        let hookRest = channelRest.testSuite_injectIntoMethod(after: #selector(RestChannel.history(_:callback:))) {
            restChannelHistoryMethodWasCalled = true
        }
        defer { hookRest.remove() }

        let hookRealtime = channelRealtime.testSuite_injectIntoMethod(after: #selector(RestChannel.history(_:callback:))) {
            restChannelHistoryMethodWasCalled = true
        }
    ...
    func test__Channel_options__setOptions__shouldUpdateOptionsOfRestChannel() throws {
        ...
        let token = channel.internal.restChannel.testSuite_getArgument(from: #selector(ARTRestChannelInternal.setOptions_nosync(_:)), at: 0) { arg in
            guard let optionsArg = arg as? ChannelOptions else {
                XCTFail("Expected setOptions: to have been called with an ChannelOptions instance")
                return
            }
    ...
    func test__138__Channel__crypto__if_configured_for_encryption__channels_encrypt_and_decrypt_messages__data() throws {
        ...
        let clientSender = Realtime(options: options)
        defer { clientSender.close() }
        clientSender.connect()

        let clientReceiver = Realtime(options: options)
    ...
    func test__002__Channel__should_not_crash_when_an_ATTACH_request_is_responded_with_a_DETACHED() throws {
        ...
        transport.setBeforeIncomingMessageModifier { protocolMessage in
            if protocolMessage.action == .attached {
                protocolMessage.action = .detached
                protocolMessage.error = ErrorInfo.create(withCode: ARTErrorCode.internalError.intValue, status: 500, message: "fake error message text")
            }
            return protocolMessage
        }

...
class RestAnnotationsTests: XCTestCase {
    ...
    func test__publish_delete_and_get_annotations() throws {
            ...
            realtimeChannel.subscribe { message in
                if message.action == .create {
                    ...
                    let annotation = ARTAnnotation(
                        id: nil, // will be generated locally, since `idempotentRestPublishing` is true by default
                        action: .create,
                        clientId: nil,
                        name: "👍",
                    ...
                    XCTAssertEqual(receivedSummary.summary?.count, 1)
        ...
        waitUntil(timeout: testTimeout) { done in
            let deleteAnnotation = ARTAnnotation(
                id: nil, // should have a different id
                action: .delete,
                clientId: createdAnnotation.clientId,
                name: createdAnnotation.name,
                count: createdAnnotation.count,
                data: createdAnnotation.data,
                encoding: createdAnnotation.encoding,
                timestamp: Date(),
                serial: createdAnnotation.serial,
                messageSerial: createdAnnotation.messageSerial,
    ...
    func test__idempotent_publishing_should_publish_annotation_with_implicit_id_only_once() throws {
        ...
        waitUntil(timeout: testTimeout) { done in
            ...
            realtimeChannel.subscribe { message in
                if message.action == .create {
                    ...
                    // When message is received, create and publish annotation via REST
                    let annotation = ARTAnnotation(
                        id: nil, // will be generated locally
                        action: .create,
                        clientId: nil,
                        name: "👍",
                        count: NSNumber(value: 10),
        ...
        // RSAN1c4: Now publish the created annotation again (same id)
        waitUntil(timeout: testTimeout) { done in
            restChannel.annotations.publish(for: receivedMessage, annotation: createdAnnotation) { error in
    ...
    func test__publish_annotation_exceeding_max_message_size() throws {
        ...
        waitUntil(timeout: testTimeout) { done in
            // Create an annotation with the large string as data
            let annotation = ARTAnnotation(
                id: nil,
                action: .create,
                clientId: nil,

...
- (void)publishForMessageSerial:(NSString *)messageSerial annotation:(ARTAnnotation *)annotation callback:(ARTCallback)callback {
...
- (void)deleteForMessageSerial:(NSString *)messageSerial annotation:(ARTAnnotation *)annotation callback:(ARTCallback)callback {
...
- (void)publishAnnotation:(ARTAnnotation *)annotation
    ...
    // RSAN1c4: generate annotation identifier if `idempotentRestPublishing` is enabled
    NSString *annotationId = nil;
    if (_channel.rest.options.idempotentRestPublishing && annotation.isIdEmpty) {
    ...
    // Create a new annotation with `messageSerial` and `action` since `ARTAnnotation` is immutable
    annotation = [[ARTAnnotation alloc] initWithId:annotationId

...
- (BOOL)add:(ARTPresenceMessage *)message reason:(NSString *)reason {
    ...
    [_logger verbose:__FILE__ line:__LINE__ message:@"1279-logging (reason %@): add: %@. Comparing to latest %@", reason, _logMessage(message), _logMessage(latest)];
...
- (void)internalAdd:(ARTPresenceMessage *)message withSessionId:(NSUInteger)sessionId reason:(NSString *)reason {
    ...
    [_logger verbose:__FILE__ line:__LINE__ message:@"1279-logging (reason %@): internalAdd: %@", reason, _logMessage(message)];
...
- (void)internalRemove:(ARTPresenceMessage *)message force:(BOOL)force reason:(NSString *)reason {
    ...
    if (!force && syncInProgress) {
        ...
        [_logger verbose:__FILE__ line:__LINE__ message:@"1279-logging (reason %@): internalRemove: %@ force: %@", reason, _logMessage(message), force ? @"YES" : @"NO"];
...
- (void)reset:(NSString *)reason {
    [_logger verbose:__FILE__ line:__LINE__ message:@"1279-logging (reason %@): reset", reason];

...
/**
 Provides parameters for a request to perform an operation that may cause an `ARTRealtimeClientInternal` instance to emit a connection state change.

 `ARTRealtimeClientInternal` will incorporate this data into the `ARTConnectionStateChange` object that it emits as a result of the connection state change.
 */

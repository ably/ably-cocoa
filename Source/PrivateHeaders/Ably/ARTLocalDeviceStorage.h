...
/// belong to. On iOS the file is written with `NSFileProtectionNone` so the
/// data is readable regardless of device lock state.
///
/// On first init, data persisted by older SDK versions (in `NSUserDefaults`
/// and the keychain) is migrated into the new file, and the old entries are
/// removed.

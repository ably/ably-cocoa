...
- (nullable id)objectForKey:(NSString *)key {
    ...
    if (value == nil) {
        ...
        ARTLogDebug(_logger, @"UserDefaults read hit for key %@: %@", key, [self loggedValue:value]);
    }
    return value;
}
...
- (void)setObject:(nullable id)value forKey:(NSString *)key {
    if (value == nil) {
        ARTLogDebug(_logger, @"UserDefaults delete for key %@", key);
    }
    else {
        ARTLogDebug(_logger, @"UserDefaults write for key %@: %@", key, [self loggedValue:value]);
    }
...
- (NSString *)secretForDevice:(ARTDeviceId *)deviceId {
    ...
    if ([error code] == errSecItemNotFound) {
        ...
        ARTLogDebug(_logger, @"Device Secret read hit for device %@: %@", deviceId, [self loggedValue:value]);
    }
...
- (void)setSecret:(NSString *)value forDevice:(ARTDeviceId *)deviceId {
    ...
    if (value == nil) {
        ...
        if ([error code] == errSecItemNotFound) {
            ...
            ARTLogDebug(_logger, @"Device Secret delete for device %@", deviceId);
        }
    }
    else {
        ...
        if (error) {
            ARTLogError(_logger, @"Device Secret couldn't be written for device %@ (%@)", deviceId, [error localizedDescription]);
        }
        else {
            ARTLogDebug(_logger, @"Device Secret write for device %@: %@", deviceId, [self loggedValue:value]);
        }
    }
}

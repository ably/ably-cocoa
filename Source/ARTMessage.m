...
+ (nullable instancetype)fromEncodedString:(NSString *)jsonString channelOptions:(ARTChannelOptions *)options error:(NSError *_Nullable *_Nullable)error {
    NSData *data = [jsonString dataUsingEncoding:NSUTF8StringEncoding];
    if (data == nil) {
        if (error != nil) {
            ARTErrorInfo *errorInfo = [ARTErrorInfo createWithCode:ARTErrorUnableToDecodeMessage message:@"Invalid JSON string provided."];
            *error = errorInfo;
        }
        return nil;
    }
    NSError *parseError = nil;
    id parsedObject = [[[ARTJsonEncoder alloc] init] decode:data error:&parseError];
    if (parseError != nil) {
        if (error != nil) {
            ARTErrorInfo *errorInfo =
            [ARTErrorInfo wrap:[ARTErrorInfo createWithCode:ARTErrorUnableToDecodeMessage message:parseError.localizedFailureReason]
                       prepend:@"Failed to parse JSON string."];
            *error = errorInfo;
        }
        return nil;
    }
    if (![parsedObject isKindOfClass:[NSDictionary class]]) {
        if (error != nil) {
            ARTErrorInfo *errorInfo = [ARTErrorInfo createWithCode:ARTErrorUnableToDecodeMessage message:@"Invalid JSON object provided: dictionary expected."];
            *error = errorInfo;
        }
        return nil;
    }
    return [self fromEncodedJsonObject:parsedObject channelOptions:options error:error];
}

+ (nullable NSArray<ARTMessage *> *)fromEncodedArrayString:(NSString *)jsonString channelOptions:(ARTChannelOptions *)options error:(NSError *_Nullable *_Nullable)error {
    NSData *data = [jsonString dataUsingEncoding:NSUTF8StringEncoding];
    if (data == nil) {
        if (error != nil) {
            ARTErrorInfo *errorInfo = [ARTErrorInfo createWithCode:ARTErrorUnableToDecodeMessage message:@"Invalid JSON string provided."];
    ...
    if (![parsedObject isKindOfClass:[NSArray class]]) {
        if (error != nil) {
            ARTErrorInfo *errorInfo = [ARTErrorInfo createWithCode:ARTErrorUnableToDecodeMessage message:@"Invalid JSON object provided: array expected."];
            *error = errorInfo;
        }
        return nil;
    }
    return [self fromEncodedArray:parsedObject channelOptions:options error:error];
}

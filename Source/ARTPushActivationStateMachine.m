...
- (instancetype)initWithRest:(ARTRestInternal *const)rest
    ...
    if (self = [super init]) {
        ...
        if ([_current isKindOfClass:[ARTPushActivationStateWaitingForPushDeviceDetails class]] && [rest loadDevice].apnsDeviceToken != nil) {
...
- (void)syncDevice {
    #if TARGET_OS_IOS
    ARTLocalDevice *const local = _rest.loadDevice;

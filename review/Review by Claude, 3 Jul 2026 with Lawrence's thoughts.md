...
> LAWRENCE: Yes, we should implement the enum approach; motivation should be justified using the referenced comment on AIT-1023
...
> LAWRENCE: Yes, we should have it for PathObject (by the way I've suggested renaming `ValueType` to `TypeOfValue` on #491 but Sachin can decide)
...
> LAWRENCE: Yes let's follow the spec here
...
> LAWRENCE: I'm happy with the enum; it avoids an explosion of PathObject types and allows us to use a consistent type for writes and reads. Decision should be documented as part of this work.
...
> LAWRENCE: The `PathObject` implementation shouldn't be in an extension. Second point goes away once we switch to enum for `Instance`
...
> LAWRENCE: I think that when I created the existing API I didn't really think about computational complexity. The Swift API Design Guidelines do [say](https://www.swift.org/documentation/api-design-guidelines/#conventions):
> 
> > Document the complexity of any computed property that is not O(1). People often assume that property access involves no significant computation, because they have stored properties as a mental model. Be sure to alert them when that assumption may be violated.
> 
> which I don't think I did document. I just thought that a bunch of stuff looked like a property so went with it. Claude's suggestion is to keep them properties on `Instance` (where they are O(1) because `size` / `keys` / `values` etc are all just reading values off the `Instance`, matching Swift's own collection idiom, and it suggests making them methods on `PathObject` because there everything involves a resolution so things are O(path length), i.e. it involves "significant computation". What do you think?
...
> LAWRENCE: The self-deregistration from inside a listener pattern was intentional; it was a solution to https://github.com/ably/ably-liveobjects-swift-plugin/issues/97, but it is admittedly ugly. If we decide to go back to not having a good answer for how to unsubscribe from within a listener then we should mention in #97 that we've done this
> LAWRENCE: Let's have the `AsyncSequence` subscription variants
...
> LAWRENCE: Why the rename of the `CounterInc.number`?
...
LAWRENCE: Yes, we should address the changes mentioned here and make the decisions clear in the PR to facilitate reviews of future iterations

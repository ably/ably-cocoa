...
class ReadmeExamples : XCTestCase {
    func testMakeKeyInstance() {
        let client = ARTRealtime(key: "xxxx:xxxx")
        client.connection.close()
    }
    
    func testMakeTokenInstance() {
        let client = ARTRealtime(token: "xxxx")
        client.connection.close()
    }
    
    func testListenToConnectionStateChanges() {
    ...
    func testSubscribeAndPublishingToChannel() {
        let options = AblyTests.clientOptions(requestToken: true)
        let client = ARTRealtime(options: options)
        defer { client.close() }
    ...
    func testQueryingThePresenceHistory() {
        let options = AblyTests.clientOptions(requestToken: true)
        let client = ARTRealtime(options: options)
        defer { client.close() }
    ...
    func testMakeRestClientAndChannel() {
        let client = ARTRest(key: "xxxx:xxxx")
        let channel = client.channels.get("test")
        _ = channel
    }
    
    func testRestPublishMessage() {
        let options = AblyTests.clientOptions(requestToken: true)
    ...
    func testRestQueryingTheHistory() {
        let options = AblyTests.clientOptions(requestToken: true)
        let client = ARTRest(options: options)
        let channel = client.channels.get("test")
    ...
    func testRestPresenceOnAChannel() {
        let options = AblyTests.clientOptions(requestToken: true)
        let client = ARTRest(options: options)
        let channel = client.channels.get("test")
    ...
    func testRestQueryingThePresenceHistory() {
        let options = AblyTests.clientOptions(requestToken: true)
        let client = ARTRest(options: options)
        let channel = client.channels.get("test")

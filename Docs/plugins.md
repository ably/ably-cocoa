...
- Everything in ably-cocoa that depends on `_AblyPluginSupportPrivate` is gated behind `#ifdef ABLY_SUPPORTS_PLUGINS`. `Package.swift` defines that macro for the `AblyPubSubDevice` target, so plugin support is compiled into every build of this package. The gate is what lets `Source` compile at all without `_AblyPluginSupportPrivate`, which is a target of this package rather than a library a build can pick up from anywhere else.

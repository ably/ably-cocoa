...
# -t / --github-access-token <token>: An access token used for fetching the logs from the GitHub API. I found that for ably-cocoa, I needed this token to have the public_repo scope in order to be able to use the .../logs endpoint.
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        -t|--github-access-token) github_access_token="$2"; shift ;;
...
then
  echo "You need to specify the GitHub access token (-t / --github-access-token)." 2>&1
  exit 1
fi
...
# 3. From the test observability server API, fetch the upload, to find the GitHub run ID, attempt number, job name, and iteration.
...
then
  ...
  # 7. From the GitHub API, fetch the logs for this job and cache them.

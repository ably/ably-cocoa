...
if [[ -z $TEST_OBSERVABILITY_SECRET ]]
then
  echo "The TEST_OBSERVABILITY_SECRET environment variable must be set." 2>&1
  exit 1
fi
...
if [[ -z $iteration ]]
then
  echo "You need to specify the current iteration number (-i / --iteration)." 2>&1
  exit 1
fi
...
# TODO check this works, I'm not convinced
# TODO how does error handling work in those subshells? also some I might want to fail e.g. the `find`
...
jq -n \
  ...
  '{ junit_report_xml: $junit_report_xml | @base64, github_sha: $github_sha, github_ref_name: $github_ref_name, github_retention_days: $github_retention_days, github_action: $github_action, github_run_number: $github_run_number, github_run_attempt: $github_run_attempt, github_run_id: $github_run_id, github_base_ref: $github_base_ref, github_head_ref: $github_head_ref, github_job: $github_job, iteration: $iteration }' \
...
curl -vvv --fail --data-binary "@${temp_request_body_file}" --header "Content-Type: application/json" --header "Test-Observability-Secret: ${TEST_OBSERVABILITY_SECRET}" "${upload_server_base_url}/uploads"

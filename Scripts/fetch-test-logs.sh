...
# Usage:
# ./fetch-test-logs.sh --test-case-id <id> --filter [filter]
#
# or
#
# ./fetch-test-logs.sh --upload-id <id>
...
run_for_test_case() {
  ...
  test_case_json=$(curl --fail --header "Accept: application/json" "${upload_server_base_url}/test_cases/${test_case_id}")
  ...
  uploads_json=$(curl --fail --header "Accept: application/json" "${upload_server_base_url}/test_cases/${test_case_id}/uploads${filter_query}")
...
extract_logs_for_test_case() {
  ...
  test_case_log_end_marker="Test Case.*${test_class_name} ${sanitised_test_case_name}.*(passed|failed)"

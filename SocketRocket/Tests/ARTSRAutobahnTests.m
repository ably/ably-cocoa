...
#import <SocketRocket/ARTSRWebSocket.h>

#import "ARTSRTWebSocketOperation.h"
#import "ARTSRAutobahnOperation.h"
#import "ARTSRAutobahnUtilities.h"

@interface ARTSRAutobahnTests : XCTestCase
@end

@implementation ARTSRAutobahnTests
...
+ (NSArray<NSInvocation *> *)testInvocations
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (NSUInteger i = 1; i <= ARTSRAutobahnTestCaseCount(); i++) {
            NSDictionary *caseInfo = ARTSRAutobahnTestCaseInfo(i);
...
+ (SEL)addInstanceMethodForTestCaseNumber:(NSInteger)caseNumber identifier:(NSString *)identifier
{
    ...
    IMP implementation = imp_implementationWithBlock(^(ARTSRAutobahnTests *sself) {
...
+ (void)updateReports
{
    ARTSRAutobahnOperation *operation = ARTSRAutobahnTestUpdateReportsOperation(ARTSRAutobahnTestServerURL(), ARTSRAutobahnTestAgentName());
    [operation start];
...
{
    NSURL *serverURL = ARTSRAutobahnTestServerURL();
    NSString *agent = ARTSRAutobahnTestAgentName();

    NSOperationQueue *testQueue = [[NSOperationQueue alloc] init];
    testQueue.maxConcurrentOperationCount = 1;

    ARTSRAutobahnOperation *testOp = ARTSRAutobahnTestOperation(serverURL, caseNumber, agent);
    [testQueue addOperation:testOp];

    __block NSDictionary *resultInfo = nil;
    ARTSRAutobahnOperation *resultOp = ARTSRAutobahnTestResultOperation(serverURL, caseNumber, agent, ^(NSDictionary * _Nullable result) {
        resultInfo = result;
    });
    ...
    if (!ARTSRAutobahnIsValidResultBehavior(identifier, resultInfo[@"behavior"])) {

...
//RSA17e
- (void)test_revoke_tokens_with_invalid_targets_fail_with_correct_message {
    //given
    ARTRest *rest = [[ARTRest alloc] initWithKey:@"xxxx:xxxx"];
    NSArray<ARTTokenRevocationTarget *> *invalidTargets = @[[[ARTTokenRevocationTarget alloc] initWith:@"invalid" value:nil]];
    XCTestExpectation *expectation = [[XCTestExpectation alloc] initWithDescription:@"Result of async"];
    __block BOOL triggered = NO;
    [rest.auth revokeTokens:invalidTargets issuedBefore:nil callback:^(ARTTokenRevocationResponse *_Nullable response, NSError *_Nullable error) {
        triggered = YES;
        if (error) {
            XCTAssertTrue([[error localizedDescription] isEqualToString:@"Invalid RevocationTarget in targets array"]);
        } else {
            XCTFail("Expected error when targets are empty but no error returned");
        }
        [expectation fulfill];

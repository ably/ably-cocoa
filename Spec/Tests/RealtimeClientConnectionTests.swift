...
class RealtimeClientConnectionTests: XCTestCase {
    ...
    func test__139__Connection__connection_id__should_be_nil_when_sdk_is_in_closing_and_closed_states() {
        // Is @Null@ when the SDK is in the @CLOSED@, @CLOSING@, @FAILED@, or @SUSPENDED@ states
        let options = AblyTests.commonAppSetup()
        options.autoConnect = false
        let client = ARTRealtime(options: options)
        let connection = client.connection
    ...
    // RTN8c - suspended
    func test__139__Connection__connection_id__should_be_nil_when_sdk_is_in_suspended() {
        let options = AblyTests.commonAppSetup()
        options.autoConnect = false
        let client = ARTRealtime(options: options)
        let connection = client.connection
    ...
    // RTN8c - failed
    func test__139__Connection__connection_id__should_be_nil_when_sdk_is_in_failed() {
        let options = AblyTests.commonAppSetup()
        options.autoConnect = false
        let client = ARTRealtime(options: options)
        let connection = client.connection

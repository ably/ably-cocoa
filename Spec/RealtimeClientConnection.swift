    ...
    override func spec() {
        describe("Connection") {
            ...
            it("Library and version param `lib` should include the `X-Ably-Lib` header value") {
                ...
                waitUntil(timeout: testTimeout) { done in
                    client.connection.on { stateChange in
                        ...
                        case .Connected:
                            if let transport = client.transport as? TestProxyTransport, let query = transport.lastUrl?.query {
                                expect(query).to(haveParam("lib", withValue: "ios-0.8.5"))
                            }
                            else {
                                XCTFail("MockTransport isn't working")
                            }
            ...
            context("Host Fallback") {
                ...
                it("every connection is first attempted to the primary host realtime.ably.io, unless overriden in ClientOptions#realtimeHost") {
                    let options = ARTClientOptions(key: "xxxx:xxxx")
                    options.autoConnect = false
                    ...
                    expect(urlConnections).to(haveCount(2))
                    expect(NSRegularExpression.match(urlConnections[0].absoluteString, pattern: "//sandbox-realtime.ably.io")).to(beTrue())
                    expect(NSRegularExpression.match(urlConnections[1].absoluteString, pattern: "//sandbox-realtime.ably.io")).to(beTrue())
                }

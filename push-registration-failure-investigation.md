...
1. Should RSH8a1 also require clearing the identity token and persisted activation state? (The original intention, per [Paddy's comment](https://github.com/ably/docs/pull/1062#discussion_r607601935), was that the transition to `NotActivated` would result in "new local device details being created on the next activation event" — implying a clean-slate registration, not a sync attempt with stale credentials.)

2. Should the failure surface as an event rather than a direct state transition, to preserve the state machine abstraction?

3. Should ably-cocoa move the device secret out of the Keychain and into the same storage as the device ID, to avoid the availability mismatch? (Paddy suggested this in [#1109](https://github.com/ably/ably-cocoa/issues/1109#issuecomment-934163390): "we need to use a persistence mechanism for the device registration and secret that is always available." The counter-argument is the security concern raised in [ably-java#593](https://github.com/ably/ably-java/issues/593).)

...
@Suite(.serialized)
final class RealtimeObjectUTSTests {
    // MARK: - Harness
    ...
    // objects/unit/RTO23/get-returns-path-object-0 & RTO23d/get-resolves-immediately-synced-0 — RTO23d:
    ...
    // objects/unit/RTO23c/get-waits-for-synced-0 — RTO23c: get() before the sync completes waits, then
    ...
    // objects/unit/RTO23e/get-rejects-failed-0 — RTO23e/RTL33c: get() on a FAILED channel is rejected by
    ...
    // objects/unit/RTO4b — get() resolves via an ATTACHED with HAS_OBJECTS false (no-objects sync
    ...
    // objects/unit/RTO17/sync-state-events-0 — RTO17b/RTO18b1/RTO18b2/RTO18e: on(.syncing) and
    ...
    // objects/unit/RTO18/on-synced-fires-0 — RTO18: on(.synced) fires when the sync completes.
    @Test
    ...
    // objects/unit/RTO18d/duplicate-listener-0 — the SAME listener registered twice (via two on() calls)
    ...
    // objects/unit/RTO19/off-deregisters-0 — RTO19/RTO18f1: off() deregisters the listener; it is not
    ...
    // objects/unit/RTO17-RTO18/sync-event-sequences-0 (scenario "initial attach") — a first sync emits
    ...
    // objects/unit/RTO17-RTO18/sync-event-sequences-0 (scenario "re-sync on new ATTACHED") — a new
    ...
    // objects/unit/RTO17-RTO18/sync-event-sequences-0 (scenario "ATTACHED without HAS_OBJECTS") — RTO4c
    ...
    // objects/unit/RTO25b/access-throws-detached-0 & access-throws-failed-0 — an access method (keys())
    ...
    // objects/unit/RTO26b/write-throws-detached-0 & write-throws-failed-0 (+ SUSPENDED per RTO26b) — a

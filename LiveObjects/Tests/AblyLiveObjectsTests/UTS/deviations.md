# Deviations — UTS objects unit suite (`AblyLiveObjectsTests/UTS`)
...
> in the plugin's own test target (`AblyLiveObjectsTests`, all 15 specs under `UTS/`), so their
> deviations are recorded here alongside them rather than in the shared harness file
> (`Test/UTS/deviations.md`, which now covers only the rest/realtime tiers).
>
...
### `objects/unit/public_object_message.md` → `UTS/PublicObjectMessageUTSTests.swift`
...
### `objects/unit/instance.md` → `UTS/InstanceUTSTests.swift`
...
### `objects/unit/value_types.md` → `UTS/ValueTypesUTSTests.swift`
...
### `objects/unit/internal_live_counter_api.md` → `UTS/InternalLiveCounterApiUTSTests.swift`
...
- **RTLC12e1 non-finite table.** `increment(amount:)` takes a `Double`, so the `string`/`boolean`/`array`/
  ...
  pinned by `InstanceUTSTests.test_RTINS14a...`.
- **Mock-realtime adaptation.** As for InstanceUTSTests: the published COUNTER_INC is asserted; the spec's
...
### `objects/unit/path_object.md` → `UTS/PathObjectUTSTests.swift`
...
- **`type()` not `getType()`** (RTTS8 discriminator name; consistent with `InstanceUTSTests` DEV-14).
...
### `objects/unit/path_object_mutations.md` → `UTS/PathObjectMutationsUTSTests.swift`
...
### `objects/unit/internal_live_map_api.md` → `UTS/InternalLiveMapApiUTSTests.swift`
...
### `objects/unit/path_object_subscribe.md` → `UTS/PathObjectSubscribeUTSTests.swift`
...
### `objects/unit/live_object_subscribe.md` → `UTS/LiveObjectSubscribeUTSTests.swift`

`Instance#subscribe` (RTINS16) delivery is owned by the engine's apply path, so these drive the **real**
`InternalDefaultRealtimeObjects` (as `PathObjectSubscribeUTSTests` does): the standard graph is seeded into
...
- **RTLO4b6 (no side effects)** is ported in the weakened form observable in the unit fixture: subscribe
  ...
  channel/connection, as for `PathObjectSubscribeUTSTests`' RTPO19g).
- **RTLO4b4c3c** (tombstone deregisters instance subs) is ported and confirmed live; note this is the
  Instance-layer teardown — path subscriptions survive a tombstone (RTLO4b4c3c1, `PathObjectSubscribeUTSTests`).

### `objects/unit/realtime_object.md` → `UTS/RealtimeObjectUTSTests.swift`
...
- **Skipped — out of unit scope:** the publish / apply-on-ACK pipeline (RTO15, all RTO20 variants,
  ...
  OBJECT+ACK path; path-subscription dispatch (RTO24a/RTO24c1) is covered by `PathObjectSubscribeUTSTests`.

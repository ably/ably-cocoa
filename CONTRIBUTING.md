...
- [`uts.yaml`](.github/workflows/uts.yaml) runs the same lanes with `suite:uts` (`-only-testing:UTS`), covering the `UTS` target alone on all three platforms. It is the **only** place CI executes that target's tests, and therefore where the ported LiveObjects `objects` unit specs run.
  ...
  Each platform runs twice, once per entry point. The `core` leg uses the default `Ably` test plan, and builds clients with `ARTRealtimeClient(options:)`. The `device` leg uses [`Test/UTSDevice.xctestplan`](Test/UTSDevice.xctestplan), whose configuration sets `UTS_SIDE=device`, and builds them with `PubSubDevice.createClient(options:)` instead. [`Test/UTS/README.md`](Test/UTS/README.md) documents the seam.
